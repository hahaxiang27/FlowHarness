...
- When constructing slash commands from hook command names, replace dots (`.`) with hyphens (`-`). For example, `speckit.git.commit` â?`/speckit-git-commit`.
...
- â?CORRECT: `- [ ] T001 Create project structure per implementation plan`
- â?CORRECT: `- [ ] T005 [P] Implement authentication middleware in src/middleware/auth.py`
- â?CORRECT: `- [ ] T012 [P] [US1] Create User model in src/models/user.py`
- â?CORRECT: `- [ ] T014 [US1] Implement UserService in src/services/user_service.py`
- â?WRONG: `- [ ] Create User model` (missing ID and Story label)
- â?WRONG: `T001 [US1] Create model` (missing checkbox)
- â?WRONG: `- [ ] [US1] Create User model` (missing Task ID)
- â?WRONG: `- [ ] T001 [US1] Create model` (missing file path)
...
## Dashboard è§æµé¢æ¿æ´æ°

tasks å®æåï¼æ?`.harness/prompts/dashboard-updater.md` ä¸?Step 4 è§åæ´æ° `{FEATURE_DIR}/dashboard.html`ï¼?
- å¡«å `#panel-tasks` é¢æ¿ï¼ææ?Task æç»è¡¨ï¼ID/Phase/ä»»å¡åå®¹/ä¾èµ/å¯å¹¶è¡?äº§åºæä»¶ï¼? Phase åè§£ç»è®¡ + Constitution XII åè§æ£æ?
- tasks èç¹ä¾§è¾¹æ ç¶ææ¹ä¸?`done` + `ok`
