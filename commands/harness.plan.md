...
Sprint 计划完成后，按 `.harness/prompts/dashboard-updater.md` 中 Step 6 规则更新 `specs/{feature}/dashboard.html`：
- 从 `.specify/feature.json` 读取 FEATURE_DIR
- 填充 `#panel-hplan` 面板：Sprint 配置表（批次/类型/任务范围/工时/门禁）+ 全部批次任务分配
- harness.plan 节点侧边栏状态改为 `done` + `ok`
