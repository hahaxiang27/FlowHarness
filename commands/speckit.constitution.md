...
1. Report the updated `.specify/memory/constitution.md` path and a brief summary of constitution changes.
2. **Stop immediately** - do not chain requirement development, `speckit.specify`, or any Harness command in the same turn.
3. If `specs/{REQUIREMENT_ID}/dashboard-state.json` exists (workflow active), update dashboard state and hand off with `.harness/prompts/command-step-gate.md` and `.harness/prompts/step-gate-handoff.md`.
4. If no dashboard state exists, ask the user to provide a requirement when they are ready.
