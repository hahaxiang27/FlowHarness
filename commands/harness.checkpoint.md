...
checkpoint 完成后，按 `.harness/prompts/dashboard-updater.md` 中 Step 16 规则更新 `specs/{feature}/dashboard.html`：
- 填充 `#panel-checkpoint` 面板：Constitution 逐条审查表（原则/检查项/结果/扣分依据）+ 总得分 + 评级
- checkpoint 节点侧边栏状态：PASS → `done` + `ok`，CONDITIONAL → `warn` + `warn`，FAIL → `fail` + `fail`
