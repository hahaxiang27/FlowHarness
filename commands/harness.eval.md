...
eval 完成后，按 `.harness/prompts/dashboard-updater.md` 中 Step 15 规则更新 `specs/{feature}/dashboard.html`：
- 填充 `#panel-eval` 面板：四级验证结果表（L1/L2/L3/L4 状态+详情+输出摘要）
- eval 节点侧边栏状态：全部 PASS → `done` + `ok`，有 FAIL → `warn` + `warn`
